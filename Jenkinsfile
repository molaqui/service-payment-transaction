pipeline {
    ...
    environment {
        ...
        APP_REGISTRY = "enamifatimazahrae2001/payment-transaction"
        MANIFEST_FILE = "payment-transaction-service/payment-transaction-deployment.yml"
    }
    ...
    stages {
        stage('Build Artifact') {
            steps {
                script {
                    dir('service-payment-transaction') {
                        ...
                        withCredentials([string(credentialsId: 'github_manifest', variable: 'GITHUB_TOKEN')]) {
                            ...
                            dir('e-banking-api-manifest') {
                                bat """
                                   ...
                                   powershell "(Get-Content -Path '${MANIFEST_FILE}') -replace 'enamifatimazahrae2001/payment-transaction.*', 'enamifatimazahrae2001/payment-transaction:${IMAGE_TAG}' | Set-Content -Path '${MANIFEST_FILE}'"
                                    git diff
                                    git add .
                                    git commit -m "Update payment-transaction service image version to ${IMAGE_TAG}"
